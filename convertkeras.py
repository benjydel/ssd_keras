...
def convert(prevmodel,export_path,freeze_graph_binary):
   ...
   previous_model = load_model(prevmodel, custom_objects={'AnchorBoxes': AnchorBoxes,
...
#                                               'DecodeDetections': DecodeDetections,
   ...
   try:
     model= Sequential.from_config(config, custom_objects={'AnchorBoxes': AnchorBoxes,
                                               'L2Normalization': L2Normalization,
#                                               'DecodeDetections': DecodeDetections,
                                               'compute_loss': ssd_loss.compute_loss}) 
   except:
     model= Model.from_config(config, custom_objects={'AnchorBoxes': AnchorBoxes,
                                               'L2Normalization': L2Normalization,
#                                               'DecodeDetections': DecodeDetections,
                                               'compute_loss': ssd_loss.compute_loss}) 
